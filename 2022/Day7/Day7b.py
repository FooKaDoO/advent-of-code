...
def add_options(directory, options, min_required=-1):
    if directory.__class__.__name__ == "Directory":
        if directory.size >= min_required:
            options.append(directory)
        for child in directory.children:
            add_options(child, options, min_required)

first_dir.printSelf()
used = find_sum(first_dir)
total_memory = 70000000
required_free = 30000000
current_free = total_memory - first_dir.size
required_to_delete = required_free - current_free
if required_to_delete <= 0:
    print("Nothing needed to be deleted.")
else:
    options = []
    add_options(first_dir, options, required_to_delete)
    smallest = min(options, key=lambda a : a.size)
    print(smallest.size)